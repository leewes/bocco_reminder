...
async function getAccessToken() {
    ...
    .post("https://platform-api.bocco.me/oauth/token/refresh", {
...
async function getMessage(){
  ...
  try {
  const res = await axios.get(`https://platform-api.bocco.me/v1/rooms/${roomUuid}/messages`,{
    headers: {
      Authorization: `Bearer ${accessToken}`,
    },
  });
...
async function setupWebhookUrl () {
  ...
  const res = await axios.post("https://platform-api.bocco.me/v1/webhook", {  
...
async function setupWebhookEvent () {
  ...
  const res = await axios.put("https://platform-api.bocco.me/v1/webhook/events", {  
    "events" :  [
      "message.received"
    ]
  }, {
...
async function sendMessage(){
  ...
  await axios.post(`https://platform-api.bocco.me/v1/rooms/${roomUuid}/messages/text`, {  
    "text" :  "おめでとう"
  }, {
    headers: {
      Authorization: `Bearer ${accessToken}`,
...
app.post("/api/hook", (req, res) => {
  ...
  //  if (type === "audio" && message.includes("宿題") && message.includes("終わった")) {
   if (message.includes("宿題") && message.includes("終わった")) {
        sendMessage();
    }
  }
})
